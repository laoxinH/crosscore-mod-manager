...
fun PermissionRequestDialog(
    ...
    when (state.permissionType) {
        ...
        PermissionType.NOTIFICATION -> {
            NotificationPermissionDialog(onConfirm = { /* 会通过其他方式处理 */}, onDismiss = onDismiss)
        }
        PermissionType.NONE -> {
            /* 不显示 */
        }
...
fun StoragePermissionDialog(onConfirm: () -> Unit = {}, onDismiss: () -> Unit) {
    AlertDialog(
            ...
            confirmButton = {
                TextButton(onClick = onConfirm) { Text(stringResource(R.string.dialog_button_confirm)) }
            },
            dismissButton = {
                TextButton(onClick = onDismiss) { Text(stringResource(R.string.dialog_button_request_close)) }
            }
    )
}
                ...
                Column {
                    ...
                    if (osVersion == OSVersion.OS_11 || osVersion == OSVersion.OS_13) {
                        SettingItem(
                                ...
                                onClick = { onRequestSAF(requestPath) }
                        )
                    }
