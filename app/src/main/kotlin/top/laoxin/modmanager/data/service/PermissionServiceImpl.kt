...
constructor(@param:ApplicationContext private val context: Context) : PermissionService {
    ...
    private val requestIdGenerator = AtomicInteger(100) // 从100开始避免与其他requestCode冲突
    ...
    /** 请求 Shizuku 权限 (挂起函数) */
    override suspend fun requestShizukuPermission(): Result<Boolean> {
        return try {
            if (!isShizukuInstalled()) {
                Result.Error(AppError.PermissionError.ShizukuNotInstalled)
            } else if (!Shizuku.pingBinder()) {
                ...
                // 使用唯一请求码
                val requestCode = requestIdGenerator.incrementAndGet()
