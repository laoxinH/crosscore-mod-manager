...
constructor(
    ...
    init {
        ...
        scope.launch(Dispatchers.IO) {
            dataStore
                    ...
                    .catch {
                        if (it is IOException) {
                            Log.e("UserPreferencesRepo", "Error reading selected game.", it)
                            emit(emptyPreferences())
                        } else {
                            throw it
                        }
                    ...
                    .map { preferences ->
                        ...
                        if (jsonString.isNullOrEmpty()) {
                            ...
                            try {
                                json.decodeFromString<GameInfoBean>(jsonString)
                            } catch (e: Exception) {
                                Log.e(
                                        "UserPreferencesRepo",
    ...
    override suspend fun prepareAndSetModDirectory(
        ...
        return withContext(Dispatchers.IO) {
            try {
                ...
                if (gameConfigFile.absolutePath.contains("${PathConstants.ROOT_PATH}/Android")) {
                    return@withContext Result.Error(AppError.FileError.PermissionDenied)
                }
                ...
                if (currentGame.packageName.isNotEmpty()) {
                    gameModsFile.mkdirs()
                    saveSelectedGame(currentGame.copy(modSavePath = gameModsFile.absolutePath))
                }
                ...
                Result.Error(
                        AppError.FileError.Unknown(e.message ?: "Failed to prepare mod directory")
                )
            }
        }
    }
    ...
    override val scanDirectoryMods: Flow<Boolean> = getPreference(Keys.SCAN_DIRECTORY_MODS, true)
    ...
    override val modsViewIndex: Flow<Int> = getPreference(Keys.MODS_VIEW_INDEX, NavigationIndex.MODS_BROWSER.index)
    override suspend fun saveModsViewIndex(index: Int) = savePreference(Keys.MODS_VIEW_INDEX, index)

    override val modListDisplayMode: Flow<Int> = getPreference(Keys.MOD_LIST_DISPLAY_MODE, 0)
    override suspend fun saveModListDisplayMode(mode: Int) = savePreference(Keys.MOD_LIST_DISPLAY_MODE, mode)
